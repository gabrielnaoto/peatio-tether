RSpec.describe Peatio::Tether::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Peatio::Tether::Blockchain.new
      expect(blockchain1.features).to eq Peatio::Tether::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Peatio::Tether::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it 'custom feautures' do
      blockchain3 = Peatio::Tether::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::Tether::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::Tether::Blockchain.new }
  ...
  context :latest_block_number do
    ...
    let(:blockchain) do
      Peatio::Tether::Blockchain.new.tap {|b| b.configure(server: server)}
    end
  ...
  context :build_transaction do
    ...
    context 'three vout tx' do
      ...
      let(:blockchain) do
        Peatio::Tether::Blockchain.new.tap { |b| b.configure(currencies: [currency]) }
      end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Peatio::Tether::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:server_without_authority) { 'http://127.0.0.1:19332' }
    let(:blockchain) do
      Peatio::Tether::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
